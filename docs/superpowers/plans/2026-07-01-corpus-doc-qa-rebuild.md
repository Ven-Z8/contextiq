...
**Vendor decision (per Venkat):** one vendor — **OpenRouter** — for both parse and answer. Parse = `mistral-ocr` engine ($2/1k pages; free `cloudflare-ai` engine for dev). Answer = **`minimax/minimax-m3`** (1M ctx, ~$0.001/query; $15 credits loaded). Call OpenRouter over **httpx** (NOT the OpenAI SDK — CLAUDE.md hard rule). Keep the existing `AnthropicLLMClient` as an optional seam for airtight Citations-API grounding.

**Architecture (lazy corpus variant):**
`parse (OpenRouter mistral-ocr) → chunk ~800tok → qdrant hybrid dense+BM25 (installed) → retrieve top ~40 → minimax-m3 answer with chunk-ID citations + abstain gate.`
...
- minimax-m3: $0.30/$1.20 per M (promo), 1M ctx ([model](https://openrouter.ai/minimax/minimax-m3)).
...
## Phase 1 — Answer path: OpenRouter/minimax-m3 + chunk-ID citations + abstain
...
- [x] **End-to-end proven**: ingest `sample-contract.md` → `hybrid_hits` (9 sources) → **minimax-m3** grounded answer with per-claim chunk-id citations + a real Confidence/Gaps block; empty-context path correctly returns `NOT_IN_DOCUMENT`.
...
2. **Contextual-retrieval prefixing** — 1-sentence LLM context per chunk pre-embed (−49% failures). Ingest-time cost.
3. **Per-doc long-context routing** — route to the winning doc, stuff the whole doc (<200K) into minimax-m3. The 79% path for single-doc questions.
