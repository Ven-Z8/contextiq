...
def test_context_engine_trims_structured_code_table_sources(tmp_path: Path) -> None:
    store = LocalDocumentStore(path=tmp_path / "blocks.json")
    store.save_blocks(
        [
            DocumentBlock(
                ...
                text=(
                    ...
                    "| UC-T-202 L | Transportation of large cargo from Earth "
                    "to the lunar surface |\n"
                    "| UC-T-203 L | Transportation of cargo from cislunar space back to Earth |"
                ),
            )
        ]
    )

    packet = ContextEngine(store=store).build_context("What is UC ID UC-T-202 L ?")

    source_text = packet.sources[0].block.text
    assert "UC-T-202 L" in source_text
    assert "Transportation of large cargo from Earth to the lunar surface" in source_text
    assert "UC-T-201 L" not in source_text
    assert "UC-T-203 L" not in source_text


def test_context_engine_drops_duplicate_structured_code_excerpts(tmp_path: Path) -> None:
    store = LocalDocumentStore(path=tmp_path / "blocks.json")
    duplicate_text = (
        "| UC ID | Use Cases |\n"
        "| --- | --- |\n"
    ...
    assert [source.block.block_id for source in packet.sources] == ["nasa:sheet-a"]


def test_context_engine_trims_asset_mapping_table_sources(tmp_path: Path) -> None:
    store = LocalDocumentStore(path=tmp_path / "blocks.json")
    store.save_blocks(
        [
            DocumentBlock(
                document_id="nasa",
                block_id="nasa:asset-functions",
                source_path="2024-lunar-objective-decomposition.xlsx",
                section_path=["Asset-Function Table"],
                block_type=BlockType.TABLE,
                text=(
                    "|  | ASSET MAPPING TO SCIENCE/RESEARCH PAYLOADS AND TECH "
                    "DEMOS IS FORWARD WORK |  |  |  |\n"
                    "| --- | --- | --- | --- | --- |\n"
                    "|  | Asset ID | Assets | FN ID | Functions |\n"
                    "|  | EM-002-HLR | Exploration Ground Systems | FN-G-101 L | "
                    "Provide ground services on Earth |\n"
                    "|  | EM-003-HLR | Orion | FN-T-101 L | Transport crew from "
                    "Earth to cislunar space |\n"
                    "|  |  |  | FN-L-302 L | Manage waste from habitable asset(s) "
                    "in cislunar space |\n"
    ...
    packet = ContextEngine(store=store).build_context(
        "What HLR functions support for Orion?"
    )

    source_text = packet.sources[0].block.text
    assert "FORWARD WORK" not in source_text
    assert "EM-003-HLR" in source_text
    assert "FN-T-101 L" in source_text
    assert "FN-L-302 L" in source_text
    assert "EM-002-HLR" not in source_text
    assert "EM-003-FE" not in source_text
